...
final class EmergencyContactsViewController: UIViewController, ViewModelContaining {
    ...
    private lazy var dataSource = RxTableViewSectionedReloadDataSource<EmergencyContactsSectionModel>(configureCell: { _, tableView, indexPath, item -> UITableViewCell in
        switch item {
        case .header:
            let cell: EmergencyContactsHeaderTableViewCell = tableView.dequeueReusableCell(for: indexPath)
//            cell.set(with: text, eventCount: eventCount)
            return cell
        case let .eventList(events):
            let cell: EmergencyContactsEventListTableViewCell = tableView.dequeueReusableCell(for: indexPath)
            cell.set(with: events, selectedEvent: { [weak self] event in
                self?.showEvent(event)
            })
            return cell
        case let .event(event):
            let cell: EmergencyContactsEventTableViewCell = tableView.dequeueReusableCell(for: indexPath)
            cell.set(with: event)
            return cell
        }
        })
...
private extension EmergencyContactsViewController {
    ...
    func setupIBOutlets() {
        fakeNavbarTitleLabel.attributedText = R.string.localizable.emergencyContactsTitle().styled(with: .dashboardActivityTitle)
        ...
        emergencyContactsTableView.register(EmergencyContactsHeaderTableViewCell.self)
        emergencyContactsTableView.register(EmergencyContactsEventTableViewCell.self)
        emergencyContactsTableView.register(EmergencyContactsEventListTableViewCell.self)
        emergencyContactsTableView.registerHeaderFooterView(SimpleSectionHeader.self)
    }

    func showEvent(_ event: EmergencyContactsResponse.Event) {
